...
function relTime(ts: number): string {
  ...
  if (diff < 60) return t("instanceCard.relTime.justNow");
  if (diff < 3600) return t("instanceCard.relTime.minutesAgo", { n: Math.floor(diff / 60) });
  if (diff < 86400) return t("instanceCard.relTime.hoursAgo", { n: Math.floor(diff / 3600) });
  if (diff < 172800) return t("instanceCard.relTime.yesterday");
  if (diff < 259200) return t("instanceCard.relTime.dayBeforeYesterday");
  if (diff < 86400 * 30) return t("instanceCard.relTime.daysAgo", { n: Math.floor(diff / 86400) });
  return new Date(ts * 1000).toLocaleDateString();
}

/** 累计游玩时长（秒）的简短人类可读格式 */
function fmtDuration(secs: number): string {
  if (secs < 60) return t("instanceCard.duration.lessThanMinute");
  const totalMin = Math.floor(secs / 60);
  const h = Math.floor(totalMin / 60);
  const m = totalMin % 60;
  if (h >= 24) return t("instanceCard.duration.daysHours", { d: Math.floor(h / 24), h: h % 24 });
  if (h > 0) return m > 0 ? t("instanceCard.duration.hoursMinutes", { h, m }) : t("instanceCard.duration.hours", { h });
  return t("instanceCard.duration.minutes", { m });
}

async function launch() {
  try {
    await instances.launch(props.instance.id);
    message.success(t("instanceCard.launched"));
  } catch (e) {
...
function confirmDelete() {
  confirmState.value = {
    title: t("instanceCard.deleteTitle"),
    content: t("instanceCard.deleteContent", { name: props.instance.name }),
    positiveText: t("instanceCard.deleteConfirm"),
    onOk: async () => {
      try {
        await instances.remove(props.instance.id);
        message.success(t("instanceCard.deleted"));
      } catch (e) {
        message.error(String(e));
      }
    },
...
<template>
  <div class="inst-card glass clickable" :class="{ 'menu-open': menuOpen }" @click="router.push(`/instance/${instance.id}`)">
    ...
    <div class="card-foot">
      <div class="foot-info">
        <span v-if="instance.last_played" :title="t('instanceCard.lastPlayedTitle', { time: new Date(instance.last_played * 1000).toLocaleString() })">
          {{ t("instanceCard.lastPlayedRel", { time: relTime(instance.last_played) }) }}
        </span>
        <span v-if="instance.last_played && instance.total_play_time > 0">·</span>
        <span v-if="instance.total_play_time > 0" :title="t('instanceCard.totalPlaytime')">
          {{ t("instanceCard.played", { duration: fmtDuration(instance.total_play_time) }) }}
        </span>
      </div>
      <div class="actions" @click.stop>
        <button
          class="icon-btn play"
          :title="t('instanceCard.launch')"
          @click="launch"
        >
          <IconPlay />
        </button>
        <button class="icon-btn" :title="t('instanceCard.openFolder')" @click="apiOpen">
          <IconFolder />
        </button>
        <div class="more-wrap">
          <button
          ...
          <Transition name="menu-pop">
            <div v-if="menuOpen" class="more-menu" :class="{ up: menuUp }">
              ...
              >
                <IconMapPin />
                <span>{{ pins.isPinned(homePinId) ? t("instanceCard.unpinHome") : t("instanceCard.pinHome") }}</span>
              </button>
              <button
                class="more-item"
              ...
              >
                <IconLayout />
                <span>{{ pins.isPinned(sidebarPinId) ? t("instanceCard.unpinSidebar") : t("instanceCard.pinSidebar") }}</span>
              </button>
              <button class="more-item" @click="runMenu(() => emit('move', instance))">
                <IconLayers />
                <span>{{ t("instanceCard.moveToGroup") }}</span>
              </button>
              <div class="more-divider"></div>
              <button class="more-item danger" @click="runMenu(confirmDelete)">
                <IconTrash />
                <span>{{ t("instanceCard.deleteInstance") }}</span>
              </button>
            </div>
          </Transition>
        </div>
  ...
  >
    <div v-if="confirmState" style="display: flex; flex-direction: column; gap: 16px;">
      ...
      <div style="display: flex; justify-content: flex-end; gap: 10px;">
        <n-button @click="confirmState = null">{{ t("instanceCard.cancel") }}</n-button>
